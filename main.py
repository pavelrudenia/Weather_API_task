...
API_KEY = '6d9a5f95dd5a25412ccbed33fc7cee03'
...
root.resizable(0, 0)
style=ttk.Style()
style.configure("TLabel",padding=5,font="Arial 17")


def getweather(event=''):
    if not entry.get():
        messagebox.showwarning("Warning", 'Введите запрос в формате город,код страны')
    else:
        params = {
            "appid": API_KEY,
            "q": entry.get(),
...
btn = ttk.Button(top_frame, text='Запрос погоды', command=getweather)
btn.place(relx=0.7, relwidth=0.3, relheight=1)

label = ttk.Label(lower_frame, anchor='nw')
label.place(relwidth=1, relheight=1)

entry.bind("<Return>",getweather)
root.mainloop()
